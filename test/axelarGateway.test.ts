...
describe("AxelarGateway", () => {
  ...
  beforeEach(async () => {
    ...
    gateway = await AxelarGateway.deploy(await owner.getAddress(), await axlToken.getAddress(), await realToken.getAddress())
  ...
  describe("pausing functionality", () => {
    ...
    it("should not allow non-pauser to pause or unpause", async () => {
      await expect(gateway.connect(user).pause()).to.be.revertedWith(
        "AccessControl: account " + (await user.getAddress()).toLowerCase() + " is missing role " + (await gateway.PAUSER_ROLE())
      )
      ...
      await expect(gateway.connect(recipient).unpause()).to.be.revertedWith(
        "AccessControl: account " + (await recipient.getAddress()).toLowerCase() + " is missing role " + (await gateway.UNPAUSER_ROLE())
      )
    })
  })
  ...
  describe("swap functionality", () => {
    ...
    it("should not allow swapping with zero amount", async () => {
      await expect(gateway.connect(user).swapToReal(0)).to.be.revertedWith("AxelarGateway: AMOUNT_MUST_BE_GREATER_THAN_0")
      await expect(gateway.connect(user).swapToAxl(0)).to.be.revertedWith("AxelarGateway: AMOUNT_MUST_BE_GREATER_THAN_0")
    })
    ...
    it("should not allow swapping if contract has insufficient balance", async () => {
      const amount = 10000
      await axlToken.connect(user).approve(await gateway.getAddress(), amount)
      await expect(gateway.connect(user).swapToReal(amount)).to.be.revertedWith("ERC20: transfer amount exceeds balance")
    })
  })
  describe("deposit & withdraw functionality", () => {
    ...
    it("should not allow depsiting with zero amount", async () => {
      await expect(gateway.connect(user).deposit(0, 0)).to.be.revertedWith("AxelarGateway: TOTAL_DEPOSIT_MUST_BE_GREATER_THAN_0")
    })
    ...
    it("should not allow withdrawing with zero amount", async () => {
      await expect(gateway.connect(user).withdraw(0, 0)).to.be.revertedWith("AxelarGateway: TOTAL_WITHDRAWAL_MUST_BE_GREATER_THAN_0")
    })
