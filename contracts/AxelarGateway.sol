...
contract AxelarGateway is ReentrancyGuard, AccessControlEnumerable, Pausable {
    ...
    constructor(
        ...
        require(
            admin != address(0),
            "AxelarGateway: ADMIN_ADDRESS_MUST_BE_NON-ZERO"
        );
        require(
            axlToken_ != address(0),
            "AxelarGateway: AXL_TOKEN_ADDRESS_MUST_BE_NON-ZERO"
        );
        require(
            realToken_ != address(0),
            "AxelarGateway: REAL_TOKEN_ADDRESS_MUST_BE_NON-ZERO"
        );
    ...
    function swapToRealTo(uint256 amount, address to)
        external
        nonReentrant
        whenNotPaused
    {
    ...
    function swapToAxlTo(uint256 amount, address to)
        external
        nonReentrant
        whenNotPaused
    {
    ...
    function deposit(uint256 realTokenAmount, uint256 axlTokenAmount)
        external
        nonReentrant
        whenNotPaused
    {
        require(
            realTokenAmount + axlTokenAmount > 0,
            "AxelarGateway: TOTAL_DEPOSIT_MUST_BE_GREATER_THAN_0"
        );

        if (realTokenAmount > 0) {
            IERC20(realToken).transferFrom(
                msg.sender,
                address(this),
                realTokenAmount
            );
    ...
    function withdraw(uint256 realTokenAmount, uint256 axlTokenAmount)
        external
        nonReentrant
        whenNotPaused
    {
